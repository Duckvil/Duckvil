...
//     }
    ...
    template <typename Type, typename... Args>
    static inline Type* fixed_stack_emplace_back(const fixed_stack_container* _pMemory, fixed_stack_allocator* _pAllocator, Args&&... _vData)
    {
        return new(_pMemory->m_fnAllocateSize(_pAllocator, sizeof(Type), alignof(Type))) Type(std::forward<Args>(_vData)...);
    }

    template <typename Type>
    static inline Type* fixed_stack_allocate(const fixed_stack_container* _pMemory, fixed_stack_allocator* _pAllocator, const Type& _data)
    {
    ...
    template <typename Type>
    static inline Type* fixed_stack_allocate(const fixed_stack_container* _pMemory, fixed_stack_allocator* _pAllocator, Type&& _data)
    {
        return fixed_stack_emplace_back<Type>(_pMemory, _pAllocator, std::move(_data));
    }

    static inline void* fixed_stack_top(const fixed_stack_container* _pMemory, fixed_stack_allocator* _pAllocator)
    {
        return _pMemory->m_fnTop(_pAllocator);
    }

    template <typename Type>
    static inline Type* fixed_stack_top(const fixed_stack_container* _pMemory, fixed_stack_allocator* _pAllocator)
    {
        return static_cast<Type*>(fixed_stack_top(_pMemory, _pAllocator));
    }

    static inline void fixed_stack_pop(const fixed_stack_container* _pMemory, fixed_stack_allocator* _pAllocator)
    {
    ...
    static inline bool fixed_stack_empty(const fixed_stack_container* _pMemory, fixed_stack_allocator* _pAllocator)
    {
        return _pMemory->m_fnEmpty(_pAllocator);
    }

    static inline bool fixed_stack_full(const fixed_stack_container* _pMemory, fixed_stack_allocator* _pAllocator)
    {
        return _pMemory->m_fnFull(_pAllocator);
    }

    static inline std::size_t fixed_stack_size(const fixed_stack_container* _pMemory, fixed_stack_allocator* _pAllocator)
    {
        return _pMemory->m_fnSize(_pAllocator);
    }

    static inline std::size_t fixed_stack_capacity(const fixed_stack_container* _pMemory, fixed_stack_allocator* _pAllocator)
    {
        return _pMemory->m_fnCapacity(_pAllocator);
    }

    static inline void fixed_stack_clear(const fixed_stack_container* _pMemory, fixed_stack_allocator* _pAllocator)
    {
